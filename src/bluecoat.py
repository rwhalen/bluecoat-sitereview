class SiteReviewRequest(object):
    
    import requests
      

    def __init__(self, URL, Category='', Email=''):
    ...
    def CurrentRating(self):
        ...
        r = self.s.get(captcha_url, headers=self.headers,stream=True)
        ...
        check_status_payload = 'url=%s&captcha=%s' % (self.url,captcha)            #URL format to be used when Captcha is required.        
        r = self.s.post('https://sitereview.bluecoat.com/rest/categorization',headers=self.headers,data=check_status_payload)      #Generate HTTP POST to check current category status
    ...
    def SubmitRating(self, Category, Email=''):
        import simplejson
        ...
        r = self.s.post('https://sitereview.bluecoat.com/rest/submitCategorization',headers=self.headers,data=payload)
        response_dict = simplejson.loads(r.text)
